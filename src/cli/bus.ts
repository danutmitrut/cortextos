...
busCommand
  ...
  .action((opts: { dryRun?: boolean }) => {
    const env = resolveEnv();
    const projectDir = env.org
      ? join(env.frameworkRoot, 'orgs', env.org)
      : (env.projectRoot || env.frameworkRoot || process.cwd());
    const report = autoCommit(projectDir, opts.dryRun ?? false);
    if (report.status === 'error') {
      process.stderr.write(`auto-commit error: ${report.error_message ?? 'unknown error'}\n`);
    }
    console.log(JSON.stringify(report));
  });
